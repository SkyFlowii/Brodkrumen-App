...
function redrawAll() {
  ...
  if (pathPoints.length > 0) {
    ...
    // Return-to-start arrow from current position
    if (originSet) {
...
function onOrientation(e) {
  ...
  if (typeof heading === 'number' && isFinite(heading)) {
    lastHeadingDeg = ((heading % 360) + 360) % 360;
...
function detectStepAndAdvance() {
  ...
  const threshold = mean + Math.max(0.6, 0.8 * std);

  const minMsBetweenSteps = 280;
  if (last.m > threshold && now - lastStepTime > minMsBetweenSteps) {
    lastStepTime = now;
    stepCount += 1;
    const stepMeters = clamp(parseFloat(elements.stepLength.value || '0.75'), 0.4, 1.2);
